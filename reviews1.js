...
function buildReview(reviewDiv, review, reply) {
	...
	//locationText.innerHTML = displayLocation(review.location);
...
function buildDeletedReview(reviewDiv, review, reply) {
	...
	var subjectHTML = review.subject;
	if (!reply) {
		subjectHTML += " | " + review.stars + "/5 Stars";
	}
	...
	nameAndDate.innerHTML = review.name + " | " + time.toLocaleString();
